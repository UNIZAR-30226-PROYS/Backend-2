...
public class AlbumAddSongTest extends TestBase {
    ...
    public void testErrorsInvalidArgs() {
        JSONObject obj = new AlbumRequests().addSongToAlbum("", user.getToken().getToken(), album.getAlbumId(), song.getId());
        Assert.assertEquals("invalidArgs", obj.getString("error"));
        obj = new AlbumRequests().addSongToAlbum(null, user.getToken().getToken(), album.getAlbumId(), song.getId());
        Assert.assertEquals("invalidArgs", obj.getString("error"));

        obj = new AlbumRequests().addSongToAlbum(user.getNick(), "", album.getAlbumId(), song.getId());
        Assert.assertEquals("invalidArgs", obj.getString("error"));
        obj = new AlbumRequests().addSongToAlbum(user.getNick(), null, album.getAlbumId(), song.getId());
        Assert.assertEquals("invalidArgs", obj.getString("error"));
    }

    @Test
    public void testErrorsUnknownUser() {
        JSONObject obj = new AlbumRequests().addSongToAlbum("invalid-user", user.getToken().getToken(), album.getAlbumId(), song.getId());
        Assert.assertEquals("unknownUser", obj.getString("error"));
    }

    @Test
    public void testErrorsInvalidToken() {
        JSONObject obj = new AlbumRequests().addSongToAlbum(user.getNick(), "invalid+" + user.getToken().getToken(), album.getAlbumId(), song.getId());
        Assert.assertEquals("invalidToken", obj.getString("error"));
    }

    @Test
    public void testErrorsInvalidAlbum() {
        JSONObject obj = new AlbumRequests().addSongToAlbum(user.getNick(), user.getToken().getToken(), -1, song.getId());
        Assert.assertEquals("invalidAlbum", obj.getString("error"));
    }
    @Test
    public void testErrorsUnknownAlbum() {
        JSONObject obj = new AlbumRequests().addSongToAlbum(user.getNick(), user.getToken().getToken(), Long.MAX_VALUE, song.getId());
        Assert.assertEquals("unknownAlbum", obj.getString("error"));
    }

    @Test
    public void testErrorsInvalidSong() {
        JSONObject obj = new AlbumRequests().addSongToAlbum(user.getNick(), user.getToken().getToken(), album.getAlbumId(), -1);
        Assert.assertEquals("invalidSong", obj.getString("error"));
    }

    @Test
    public void testErrorsUnknownSong() {
        JSONObject obj = new AlbumRequests().addSongToAlbum(user.getNick(), user.getToken().getToken(), album.getAlbumId(), Long.MAX_VALUE);
    ...
    public void testAlreadyAdded() {
        ...
        obj = new AlbumRequests().addSongToAlbum(user.getNick(), user.getToken().getToken(), album.getAlbumId(), song.getId());
