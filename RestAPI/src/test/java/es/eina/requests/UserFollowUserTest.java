...
/*
    ...
    public void testErrorsInvalidArgs(){
        JSONObject obj = performTest(new UserRequests().follow("", user2.getNick()));
        Assert.assertEquals("invalidArgs", obj.getString("error"));
        obj = performTest(new UserRequests().follow(null, user2.getNick()));
        Assert.assertEquals("invalidArgs", obj.getString("error"));

        obj = performTest(new UserRequests().follow(user.getNick(), ""));
        Assert.assertEquals("invalidArgs", obj.getString("error"));
        obj = performTest(new UserRequests().follow(user.getNick(), null));
        Assert.assertEquals("invalidArgs", obj.getString("error"));
    ...
    public void testErrorsUnknownUser1(){
        JSONObject obj = performTest(new UserRequests().follow("invalid-user", user2.getNick()));
        Assert.assertEquals("user1NotExists", obj.getString("error"));
    }

    @Test
    public void testErrorsUnknownUser2(){
        JSONObject obj = performTest(new UserRequests().follow(user.getNick(), "invalid-user"));
        Assert.assertEquals("user2NotExists", obj.getString("error"));
    }

    @Test
    public void testOK(){
        JSONObject obj = performTest(new UserRequests().follow(user.getNick(), user2.getNick()));
    ...
    public void testAlreadyFollowing(){
        ...
        obj = performTest(new UserRequests().follow(user.getNick(), user2.getNick()));
